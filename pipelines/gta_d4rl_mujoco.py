...
class GTAD4RLMuJoCoTDDataset(D4RLMuJoCoTDDataset):
    def __init__(self, save_path, dataset, normalize_reward: bool = False):
        ...
        # Since gta generates normalized observations, we do not need to normalize them.
...
@hydra.main(config_path="../configs/gta/mujoco", config_name="mujoco", version_base=None)
def pipeline(args):
    set_seed(args.seed)
    ...
    nn_condition = MLPCondition(
        in_dim=1, out_dim=args.embed_dim , hidden_dims=[(args.embed_dim), ], act=nn.SiLU(), dropout=args.label_dropout
    )
    ...
    if args.mode == "train_diffusion":
        ...
        lr_scheduler = CosineAnnealingLR(gta.optimizer, T_max=args.diffusion_gradient_steps)
        ...
        for batch in loop_dataloader(dataloader):
            ...
            if (n_gradient_step + 1) % args.save_interval == 0:
                gta.save(save_path + f"diffusion_ckpt_{n_gradient_step + 1}.pt")
                gta.save(save_path + f"diffusion_ckpt_latest.pt")
    ...
    # --------------------- Train RL ------------------------
        ...
        dataset = GTAD4RLMuJoCoTDDataset(save_path, d4rl.qlearning_dataset(env), args.normalize_reward)
        dataloader = DataLoader(
            dataset, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True, drop_last=True)
